...
def plot_cylindrical_residuals(
    ...
    print("min & max vcirc_res_interp:", np.min(vcirc_res_interp), np.max(vcirc_res_interp))
    ...
    print("vcirc_res_interp nan detected!") if np.sum(np.isnan(vcirc_res_interp)) else None
    ...
    ax[0].imshow(vrad_res_interp.T, origin="lower", extent=(-8, 12, -5, 15), norm=norm_rad)
    ...
    ax[1].imshow(vcirc_res_interp.T, origin="lower", extent=(-8, 12, -5, 15), norm=norm_circ)
    ...
    ax[0].legend(loc="lower left", fontsize=9)
    ax[1].scatter(
        x, y, c=v_circ_res, norm=norm_circ, cmap=cmap, s=10, edgecolors="k", label="Masers"
    )
    ax[1].set_xlim(-8, 12)
    ax[1].set_xticks([-5, 0, 5, 10])
    ax[1].set_ylim(-5, 15)
    ...
    ax2[1].imshow(
        vcirc_res_interp_sigma.T, origin="lower", extent=(-8, 12, -5, 15), norm=norm_circ_sd
    )
