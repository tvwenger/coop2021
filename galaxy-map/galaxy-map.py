...
def barycentric_to_galactocentric(Xb, Yb, Zb, R0=_RSUN, Zsun=_ZSUN, roll=_ROLL):
    ...
    #
    # Roll CCW about the barycentric X-axis so that the X-Z plane
    # is aligned with the X-Z plane of the Galactocentric frame
    #
...
def main():
    ...
    #     acoord.Galactocentric(galcen_distance=8.15 * u.kpc, z_sun=5.5 * u.kpc)
    # )
    ...
    ax.set_aspect("equal")
    fig.savefig(
        join(dirname(abspath(__file__)), "galaxy-map.jpg"),
        format="jpg",
        dpi=300,
        bbox_inches="tight",
    )
