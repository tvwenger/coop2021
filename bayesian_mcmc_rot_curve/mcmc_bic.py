...
Calculates Bayesian Information Criterion (BIC) from
pickle file generated by MCMC_w_dist_uncer.py
...
def ln_gauss(x, mean, sigma):
    """
    Calculates the natural log of the normal distribution

    TODO: finish docstring
    """

    ln_coeff = -np.log(sigma) - 0.5 * np.log(2 * np.pi)
    ln_exp = -0.5 * (x - mean) * (x - mean) / (sigma * sigma)

    return ln_coeff + ln_exp


def ln_cauchy(x, x_peak, sigma):
    """
    Calculates the natural log of the Cauchy distribution.
    ...
    ln_hwhm_pi = 1.308047016  # ln(hwhm * pi) = ln(sqrt(2*ln(2)) * pi)

    ln_coeff = -ln_hwhm_pi - np.log(sigma)  # -ln(hwhm * pi * sigma)
    frac = (x - x_peak) / (hwhm * sigma)

    return ln_coeff - np.log(1 + frac * frac)


def ln_siviaskilling(x, mean, sigma):
    """
